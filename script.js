...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Marking touch events as passive
    ...
    joystickContainer.addEventListener('touchstart', (e) => {
        ...
        // Show the joystick and move it to where the finger touches
        joystickContainer.style.opacity = 1;
        joystickContainer.style.left = `${joystickStartX - joystickContainer.offsetWidth / 2}px`;
        joystickContainer.style.top = `${joystickStartY - joystickContainer.offsetHeight / 2}px`;
    
        joystickActive = true;
        joystickStartX = e.touches[0].clientX;
        joystickStartY = e.touches[0].clientY;
        
    // Move joystick container to where the finger touches
    joystickContainer.style.left = `${joystickStartX - joystickContainer.offsetWidth / 2}px`;
    joystickContainer.style.top = `${joystickStartY - joystickContainer.offsetHeight / 2}px`;
    ...
    joystickContainer.addEventListener('touchmove', (e) => {
        ...
        joystickDiffX = touch.clientX - joystickStartX;
        joystickDiffY = touch.clientY - joystickStartY;

        // Correcting joystick visual orientation to match movement
        const maxDiff = 50;
        const moveX = Math.max(-maxDiff, Math.min(maxDiff, joystickDiffX));
        const moveY = Math.max(-maxDiff, Math.min(maxDiff, joystickDiffY));
        
    // Moving the joystick container only, based on the touch movement
    joystickContainer.style.left = `${touch.clientX - joystickContainer.offsetWidth / 2}px`;
    joystickContainer.style.top = `${touch.clientY - joystickContainer.offsetHeight / 2}px`;
    ...
    joystickContainer.addEventListener('touchend', () => {
        ...
        joystickDiffX = 0;
        joystickDiffY = 0;
        joystick.style.transform = 'translate(0, 0)'; // Reset joystick position on release
    });
    ...
    joystickContainer.addEventListener('mousedown', (e) => {
        ...
        joystick.style.transform = 'translate(0, 0)'; // Reset joystick position on mouse down
    });

    joystickContainer.addEventListener('mousemove', (e) => {
        if (!joystickActive) return;
        joystickDiffX = e.clientX - joystickStartX;
        joystickDiffY = e.clientY - joystickStartY;
    ...
    function movePlayerWithJoystick() {
        if (joystickDiffX > 20) {
            playerMoveRight();
        } else if (joystickDiffX < -20) {
            playerMoveLeft();
        }

        // Correctly trigger the same behavior as the up arrow key when the joystick is pressed upward
        if (joystickDiffY < -20 && !player.isJumping) {
            player.isJumping = true;
            player.velocityY = -player.jumpHeight; // Same jumping logic as the up arrow
        }
    }
